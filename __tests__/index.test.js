...
const expectedStylish = readFile(getAbsFileName('expectStylish.txt'));
const expectedPlain = readFile(getAbsFileName('expectPlain.txt'));
const expectJSON = readFile(getAbsFileName('expectJSON.txt'));
...
test('Absolute paths', () => {
  ...
  expect(genDiff(absFileJSON1, absFileJSON2)).toEqual(expectedStylish);
});

test('YAML file format', () => {
  const relFileYAML1 = getRelFileName(fileYAML1);
  const relFileYAML2 = getRelFileName(fileYAML2);
  expect(genDiff(relFileYAML1, relFileYAML2)).toEqual(expectedStylish);
});
